 ...
 * points to a valid GitHub repository or GIst.
 * @param {string} [button.repository] - A link to a GitHub repository or Gist
 * that contains files for a button script.
 ...
 * @param {boolean} [button.isListed] - If present true, the button will be
 ...
 * string that can contain any characters, including emojis.
...
module.exports = function(button) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      else if (typeof button.styles == 'object') {
        button.styles = JSON.stringify(button.styles);
      }
      else {
        try {
          JSON.parse(button.styles);
        }
        catch (err) {
      ...
      if (!!+button.key) {
        button.modifierKey = rstring.generate(40);
        delete button.key;
      }
