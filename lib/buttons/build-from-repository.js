...
const atob = require('lib/util/atob');

/**
 * Validates a Github repo's data and attempts to build a button.
 * @module lib/buttons/build-from-repository
 ...
 */
module.exports = async function(url, files) {
  ...
  // Build array of files to download
    ...
    download = files
      .filter(f => f.type == 'file' && f.name != 'xybuttons.manifest.json')
      .map(f => f.name);
  }
